...
The spatial Jacobian $\log|I - \rho W|$ is evaluated at every MCMC draw. `bayespecon` supports five methods, auto-selected by problem size:
...
| `traces` | flow models | O(nnz·p) | O(p) | Multinomial trace expansion for 3-param flow models |
