"""Fast build/method/recovery tests for non-spatial Logit and NegativeBinomial."""
...
def test_negbin_rejects_noninteger_or_negative_y():
    ...
    with pytest.raises(ValueError, match="integer-valued"):
        NegativeBinomial(y=np.array([0.0, 1.2, 2.0, 1.0]), X=X)
    with pytest.raises(ValueError, match="non-negative"):
        NegativeBinomial(y=np.array([0.0, 1.0, -1.0, 2.0]), X=X)


def test_negbin_rejects_robust():
    y, X, _, _ = _count_data()
    with pytest.raises(NotImplementedError, match="robust"):
        NegativeBinomial(y=y, X=X, robust=True)


def test_negbin_priors_typed():
    y, X, _, _ = _count_data()
    m = NegativeBinomial(y=y, X=X, priors=NegBinPriors(alpha_nu=5.0, alpha_sigma=1.5))
