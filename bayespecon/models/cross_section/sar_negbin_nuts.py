r"""NUTS-sampled structural-form SAR Negative Binomial.

This is the **structural form with a latent spatially-smoothed random
effect** sampled with NUTS.  It is the NUTS analogue of
:class:`bayespecon.models.cross_section.sar_negbin_latent.SARNegBinLatent`
(which uses Pólya-Gamma Gibbs on the same structural form) and is kept
primarily for validation / sampler comparison.

Most users should prefer either

* :class:`bayespecon.models.cross_section.sar_negbin.SARNegativeBinomial`
  — the reduced-form spatial-econometric model (no :math:`\sigma`),
  sampled with Pólya-Gamma Gibbs.  This is the canonical SAR-NB.
* :class:`SARNegBinLatent` / ``SARNegativeBinomialLatent`` — same
  structural form as this class but with the much faster PG-Gibbs
  sampler.

The target posterior here is
...
the non-centred parameterisation of
...
class SARNegativeBinomialNUTS(SpatialModel):
    r"""NUTS-sampled SAR Negative Binomial (structural form with :math:`\sigma`).

    .. note::

       This class is retained primarily for validation against the
       Pólya-Gamma Gibbs samplers.  For routine use prefer
       :class:`~bayespecon.models.cross_section.sar_negbin.SARNegativeBinomial`
       (the reduced-form model — canonical spatial-econometric SAR-NB)
       or :class:`~bayespecon.models.cross_section.sar_negbin_latent.SARNegBinLatent`
       (same structural form as this class, faster Gibbs sampler).
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.robust:
            raise NotImplementedError(
                "robust=True is not supported for SARNegativeBinomialNUTS."
            )

        y_round = np.round(self._y).astype(np.int64)
        if not np.allclose(self._y, y_round):
            raise ValueError(
                "SARNegativeBinomialNUTS requires integer-valued observations."
            )
        if np.any(y_round < 0):
            raise ValueError(
                "SARNegativeBinomialNUTS requires non-negative integer observations."
            )
