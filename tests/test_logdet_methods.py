...
def test_lndetint_returns_expected_keys_and_lengths() -> None:
    W = _toy_w()
    out = lndetint(W, 1e-5, 0.5, n_grid=30)
...
def test_lndetmc_returns_confidence_bounds() -> None:
    W = _toy_w()
    out = lndetmc(order=12, iter=32, W=W, rmin=1e-5, rmax=0.5, grid=0.05, random_state=7)
...
def test_logdet_grids_match_direct_slogdet_for_full_int_ichol() -> None:
    ...
    out_full = lndetfull(W, rmin, rmax, grid=0.05)
    out_int = lndetint(W, rmin=rmin, rmax=rmax, n_grid=80)
    out_ichol = lndetichol(W, rmin, rmax, grid=0.05)
...
def test_lndetmc_tracks_exact_trend_for_symmetric_row_standardized_w() -> None:
    ...
    out = lndetmc(order=30, iter=120, W=W, rmin=1e-5, rmax=0.8, grid=0.05, random_state=0)
    I = np.eye(n)
